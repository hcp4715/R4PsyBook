...
在Match数据集中，我们有一个名为Shape的变量，它代表了社会意义。这个变量实际上包含了两个分类变量：Valence和Identity。Valence代表了情感的极性，而Identity代表了所属类别。我们需要将这两个变量分开处理，以便我们可以更好地分析数据。

我们需要将数据分成两个部分，一个是自我，一个是他人。可以使用分隔符将它们分开。在这里，我们使用分隔符来代表上面的数据点。Code等于Shape，这意味着我们要对哪个列进行分割。我们将一个列分成两个，分别命名为Valence和Identity。我们使用一种比较高级的方式来进行分割，实际上是将其分成两部分，一部分是道德或不道德，另一部分是自我或他人。
